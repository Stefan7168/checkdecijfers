...
7. Next-session prompt: hand a paste-ready kickoff AND save a durable copy to `docs/session-briefs/<date>-session-<n>-kickoff.md` (so the handoff survives a clean 0%-context restart). Then end the wrap-up reply with the one-sentence opener the owner pastes into the new chat, in its own fenced block, real path filled in: `Read CLAUDE.md, then docs/STATUS.md's top block, then docs/session-briefs/<date>-session-<n>-kickoff.md. Continue from there.`
