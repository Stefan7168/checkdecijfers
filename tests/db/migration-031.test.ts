// Migration 031 (WP30c phase E1, ADR 048 D7(a)): the additive nullable `doi`
...
describe('migration 031 is picked up by the migration scan', () => {
  it('applyMigrations records 031_source_doi.sql as applied', async () => {
    await withDb(async (db) => {
      const { rows } = await db.query(
        "select name from schema_migrations where name like '031_%' order by name",
      );
      expect(rows.map((r) => r.name)).toEqual(['031_source_doi.sql']);
    });
  });
