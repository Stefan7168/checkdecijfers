  ...
  it('still rejects a compensation reversing a signup_grant row (the guard is not disabled)', async () => {
    await withDb(async (db) => {
      ...
      await expect(
        ...
        // and migration 027 (ADR 037), which widen the allowlist to add
        // 'websearch_cost' then 'dataset_cost' — all migrations run in these
        // hermetic tests, so the message names all four permitted reasons. A
        // signup_grant is still correctly rejected either way.
