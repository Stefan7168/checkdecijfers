...
export async function runSyncFromCapture(
  ...
  let result: Awaited<ReturnType<typeof syncTable>>;
  try {
    result = await syncTable(db, source, args.tableId, {
      acceptNewCodes: args.acceptNewCodes,
      rebaseline: args.rebaseline,
    });
  } catch (err) {
    // syncTable THROWS (doesn't return a failed result) when the table is
    // already needs_review and --rebaseline wasn't passed — exactly the
    // recovery step the hint below points an operator at. Without this catch,
    // that throw propagates uncaught past a release-day retry that forgot the
    // flag, replacing the guided recovery message with a raw stack trace.
    console.error(`[${args.tableId}] FAILED: ${err instanceof Error ? err.message : String(err)}`);
    return 2;
  }
