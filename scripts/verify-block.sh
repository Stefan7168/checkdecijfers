...
wait_for_vitest() { while pgrep -f "[n]ode.*vitest" >/dev/null; do sleep 15; done; }
{
  ...
  echo "--- typecheck root ---"; npm run typecheck 2>&1 | tail -3; echo "exit=${pipestatus[1]}"
  echo "--- typecheck web ---"; npm run web:typecheck 2>&1 | tail -3; echo "exit=${pipestatus[1]}"
  wait_for_vitest
  echo "--- backend full suite (solo) $(date -u +%T) ---"; npm test 2>&1 | grep -E "Test Files|Tests |failed|FAIL|Error:" | tail -15; echo "exit=${pipestatus[1]}"
  echo "--- benchmark run $(date -u +%T) ---"; npm run benchmark:run 2>&1 | tail -3; echo "exit=${pipestatus[1]}"
  echo "--- benchmark score ---"; npm run benchmark:score 2>&1 | grep -vE "^\s+(PASS|FAIL)\s" | tail -8; echo "exit=${pipestatus[1]}"
  wait_for_vitest
  echo "--- web suite $(date -u +%T) ---"; npm run web:test 2>&1 | grep -E "Test Files|Tests |failed|FAIL" | tail -8; echo "exit=${pipestatus[1]}"
  echo "--- next build $(date -u +%T) ---"; npm run web:build 2>&1 | grep -E "Compiled|TypeScript|error|Error|✓|✗|Route \(app\)" | head -12; echo "exit=${pipestatus[1]}"
  echo "=== DONE $(date -u +%T) ==="
} > "$L" 2>&1
