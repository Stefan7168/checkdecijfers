...
| `env.sh` | The env the dev server needs (incl. `CDC_PGLITE_HARNESS=1`, which also opens the PGlite seam under a production `next start` — RUNBOOK § `CDC_PGLITE_HARNESS`) (`source` it, then `npx next dev -p 3102` from `web/`). |
