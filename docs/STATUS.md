...
review, NONE merged — this is purely a review wait, nothing blocks any of them on the tooling side:**
...
`session-briefs/2026-09-14-wp30c-e1-executor-brief.md`), plus the next-session kickoff
...
oversight). **CI on PR #23 confirmed green** (`gh pr checks 23`, run `34885657335`: `backend (1/2/3)`
and `web` all `pass`; `deploy` correctly `skipping` — PRs never deploy, only pushes to `main` do).
...
2. **Merge PR #22** (the Pro subscription tier, above) — same standing rule. Merging alone does
   NOT turn anything on for real users: `PRO_SUBSCRIPTIONS_ENABLED` stays unset until the owner
   separately works through RUNBOOK's go-live checklist (migration 030, the real Stripe Price
   object, the webhook subscription, then the flag).
