# #130 websearch — owner interview sheet (2026-07-12) — ✅ CLOSED

**All ten answered in-chat, same session (2026-07-12): owner asked to see the four questions that actually carry decision weight — pricing (Q2), web-only mode (Q4), failure/refund (Q6), domain policy (Q7) — got them with the tradeoff spelled out, replied "Go with your defaults."** That covers the remaining six too (flagged upfront as "I'll take as accepted unless you flag one" — none flagged). Every answer below is therefore the RECOMMENDED default, owner-confirmed. ADR [032](../decisions/032-websearch-augmentation.md) is now Accepted.
...
> Antwoord: AKKOORD — "Van het web (niet door checkdecijfers geverifieerd)".
...
> Antwoord: AKKOORD — +10 credits on top of the normal question price.
...
> Antwoord: AKKOORD — max ~4 findings, one sentence + source link, bordered block below the CBS body. Sub-question (favicon/domain vs full URL): domain name only — quieter, avoids raw-URL clutter.
...
> Antwoord: AKKOORD — web-only questions (CBS deselected) are allowed in v1.
...
> Antwoord: AKKOORD — the refusal text stays exactly as today; the web section may still appear below it.
...
> Antwoord: AKKOORD — one honest line + automatic refund of the web add-on credits via the existing compensation mechanism.
...
> Antwoord: AKKOORD — open web in v1, `user_location` set to Nederland, no allowlist.
...
> Antwoord: AKKOORD — one WP, chips UI first then the web channel, same branch/PR per #118.

---

**Interview closed 2026-07-12 (session 39, owner in-chat, all defaults confirmed).** ADR 032 flipped to Accepted; open-questions #130 updated. Next: the frozen executor brief (WP27/WP30b pattern — pre-build adversarial review before the build) is the remaining step before #129+#130 can be built.
