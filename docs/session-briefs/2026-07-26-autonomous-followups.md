...
| [#184](../open-questions.md) gate never checks the IP limit | **Hold.** It adds a third uncached query per anonymous page view, straight into [#186](../open-questions.md)/[#173](../open-questions.md). Sequence it AFTER a pot-read cache, never before. |
| [#185](../open-questions.md) refund restores the abuse counters | **Build the cheap half** (drop `and not refunded` from the two COUNT queries, so a refund returns the pot unit without returning the budget). Latent-only today, and the fix is smaller than the reasoning about it. |
| [#186](../open-questions.md) anonymous per-GET DB cost | **Measure first, then the 15-30 s pot-read cache.** It cannot over-serve — the atomic take is the real gate — and it preserves the owner's "a refill re-opens without a deploy" decision to within the TTL. |
