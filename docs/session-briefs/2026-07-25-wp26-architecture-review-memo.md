...
5. **Trial-gate TTL cache (5–10 s) and confirm whether the trial Anthropic key shares the org rate limit.** *(⚠ SUPERSEDED 2026-07-26: the cache was built at **20 s**, not 5-10, and on the POT READ ONLY — see [#186](../open-questions.md) for the measurement that set the number. This memo predates it; read the row, not this line. The Anthropic-key rate-limit half is still unconfirmed.)* Q5's most plausible next #173: two uncached queries per anonymous request plus one global advisory lock against a 15-connection ceiling. Price: refill re-opens seconds late. **When: before the trial go-live (#53).**
