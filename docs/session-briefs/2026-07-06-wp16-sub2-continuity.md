...
- ✅ Build worktree exists: branch `wp16-sub2` (branched from main @ `1bb8536`), checked out at `/private/tmp/claude-502/-Users-redacted-Documents-Check-de-Cijfers/2c39b13a-0681-42f9-9369-649593d26671/scratchpad/wp16-sub2`, dependencies installed. **If that tmp path is gone (reboot):** `git worktree prune && git worktree add <new-dir> wp16-sub2 && cd <new-dir> && npm ci && npm run web:ci` — the branch's committed stage work survives in the repo's own .git regardless of the worktree directory.
...
3. The workflow's live progress: `/workflows` in the session UI; scripts live under `~/.claude/projects/-Users-redacted-Documents-Check-de-Cijfers/<session-id>/workflows/scripts/`.

## How to resume after any interruption

- **Workflow died / new session:** relaunch the Workflow tool with the current script (see the script path in the session that launched it; if unknown, the script can be reconstructed — it is stage prompts pointing at the design file + this brief). If the run id is known, pass `resumeFromRunId` — completed agents return cached. Otherwise: check `git log wp16-sub2` for the last completed stage and launch a fresh workflow SKIPPING completed stages (edit the script: delete the done stages' agent calls).
