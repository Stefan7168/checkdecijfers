...
*Note on B20 ([open-questions #216](open-questions.md), fixed session 110): the "does not yet cover that month" condition is what the FROZEN key and the hermetic (CI) gate always assume — pinned fixtures + clock injection keep it true forever there. On a real `benchmark:run:live` run, CBS's actual publication lag means the condition can genuinely stop holding (the live table catches up), and a correct, attributed, non-fabricated answer is then the right outcome, not a failure. The live scorer's B20 check reflects this: it derives from the run's own recorded evidence (a real answer attributed to exactly the resolved "previous month", with zero fabricated numbers) rather than from a fixed assumption — see ADR [017](decisions/017-live-benchmark-run.md)'s session-110 addendum.*
