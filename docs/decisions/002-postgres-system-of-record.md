...
- `cbs_tables` — registry: our ID, CBS table ID, title, description, dimensions, units, update cadence, last sync, **schema fingerprint**, version.
- `observations` — long/narrow facts: table ref, measure, region code, period, value, unit, ingestion batch.
- `dimension_labels` — CBS code → human label (the "translation layer" the notes call the real IP).
