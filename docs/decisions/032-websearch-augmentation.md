...
## As-built addendum (2026-09-08, session 89 — `WEBSEARCH_MODEL` Sonnet→Haiku, same-session as ADR 013's phrasing switch)

**Owner decision, present in session ("it becomes costly"):** `WEBSEARCH_MODEL` switched from `claude-sonnet-5` to `claude-haiku-4-5`. No committed fixture is orphaned by this — this call is hermetic via a hand-authored fake (`tests/websearch/client.test.ts`), never fixture replay, and the test already asserts `params.model` dynamically against the `WEBSEARCH_MODEL` constant rather than a literal string.

**The real risk this decision carried:** the BASIC `web_search_20250305` tool variant's citation behavior was only ever MEASURED live on `claude-sonnet-5` (the go-live correction above) — the newer `20260209` variant's `citations: null` failure on Sonnet is direct precedent that a model/variant combination can silently break citations with no error, only a soft `no_findings` fallback masking it. Haiku on the BASIC variant was **verified live before this shipped**, not assumed: one real call ("Wat is het laatste nieuws over de Nederlandse arbeidsmarkt?") returned 4 findings, every one with a real, resolvable citation URL — no regression. `WEBSEARCH_MAX_TOKENS = 16_000`'s comment (sized around Sonnet 5's adaptive-thinking token sharing) was corrected to note it's unchanged-but-not-load-bearing for Haiku, since neither model gets an explicit `thinking` override at this call site.
