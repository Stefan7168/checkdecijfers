...
## As-built addendum (2026-09-08, session 89 — `PHRASING_MODEL` Sonnet→Haiku, revisit trigger "spend")

**Owner decision, present in session ("it becomes costly"):** `PHRASING_MODEL` switched from `claude-sonnet-5` to `claude-haiku-4-5`, matching every other model in the codebase (`INTENT_MODEL`, `SEMANTIC_CHECK_MODEL`, `TABLE_RERANK_MODEL`, `MEASURE_FIT_MODEL`, `DATASET_INSTRUCT_MODEL`) except `WEBSEARCH_MODEL` (switched the same session — see ADR [032](032-websearch-augmentation.md)'s own as-built addendum). This decision's own revisit trigger ("Anthropic deprecates `claude-sonnet-5`") is generalized here to "any change in the model tier," which this addendum documents end to end.

**Sampling parameters swapped, not just the model ID:** Sonnet 5 rejects `temperature: 0` and needed `thinking: 'disabled'` to opt out of adaptive thinking; Haiku is the reverse (every other Haiku-tier request in this codebase sets `temperature: 0`, none set `thinking`). `buildPhrasingRequest` now sets `temperature: 0` and no longer sets `thinking`. This is exactly the failure class [open-questions #172](../open-questions.md) already documents for the reverse swap (Haiku→Sonnet on a different model): "the un-adapted param API-errored every call."

**A real, measured phrasing gap found and fixed before shipping — `COMPOSE_PROMPT_VERSION` bumped to 4:** re-recording the answer fixtures live against Haiku (`npm run answer:record`) scored 13/14 on the first pass. The one failure (B8, a 6-period price series) was not a fabrication or a wrong number — every value Haiku used was real and correctly formatted — it was an omission: Haiku summarized the middle of the series narratively ("stegen niet in een rechte lijn... na een stijging tot X in 2022 volgde een daling naar Y in 2023") instead of stating every period's value the way rule 6 implicitly expected but never explicitly required (B4, a 5-period series, happened to get full enumeration from Sonnet's own habitual style, never because a rule demanded it). Added rule 6b: *"for a series with more than two periods, state the value of EVERY period, never only the first and last."* Re-recorded: 14/14. This is the intended process this ADR's own "Revisit triggers" section anticipated ("Benchmark answer quality below the Phase 0 gate... → revisit prompt, then model tier") — fix the prompt first, only step down the model tier if that doesn't close the gap.

**Full verification before shipping:** hermetic suite 2089/2089 (backend) + 772/772 (web), both typechecks clean, real `next build` clean, hermetic benchmark 14/14 + 6/6 + 0 fabricated (`npm run benchmark:run`/`benchmark:score`), **and** a live end-to-end run against the real Anthropic API and the real production-equivalent database (`npm run benchmark:run:live`/`benchmark:score:live`) — 14/14 answerable, 0 fabricated, but **5/6 on refusal/clarify (GATE VERDICT: FAIL)**. Root-caused before treating this as a phrasing regression: B20 ("Wat was de inflatie van vorige maand?", `reason: "freshness"`) is defined to refuse *only when the loaded CPI table does not yet cover that month* — the live run (reference date 2026-08-15, "vorige maand" = July 2026) found the real, live CBS table already synced through July 2026 (`gesynchroniseerd op 2026-08-26`), so the correct, honest behavior is to ANSWER, not refuse. The answer given was fully correct, attributed, and marked `(voorlopig cijfer)`. **This is unrelated to the model switch** — any model would correctly answer once the real data caught up to the task's assumed freshness gap; it is pre-existing benchmark-task staleness, tracked separately at [open-questions #216](../open-questions.md), not a phrasing quality regression. The hermetic gate (the actual CI gate, pinned fixtures immune to real-world data drift) passed clean throughout.

**`WEBSEARCH_MODEL`'s citation behavior on Haiku was verified live, not assumed** — see ADR 032's own addendum.

**Not done, flagged for a future session (owner's stated goal, same conversation): true model independence.** The owner asked, after seeing the B8 gap, to be able to swap models "at any time" without this kind of manual prompt-tuning cycle. The real answer already exists in this codebase as a draft: [#162's `SLOT_PHRASING_ENABLED`](../05-data-rules.md) (flag OFF everywhere) has the model write digit-free prose with typed placeholders (`{waarde1}`, `{periode1}`) that deterministic code fills from the validated result — a fabricated or dropped number becomes structurally unrepresentable regardless of which model is writing, closing exactly the class of gap B8 exposed. Turning it on is a separate, real initiative (a hermetic half is built per the flag's own doc note; the live half and a decision to flip it are not) — not something to fold into a cost-driven model swap. Tracked at [open-questions #217](../open-questions.md).
