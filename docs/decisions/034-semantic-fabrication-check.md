...
1. **Date-form metadata echoes are hard.** An echo whose matched anchor is a month name ("op **1 januari**") is a date — `<digit> <maand>` has no quantity reading in Dutch. Bracket coordinates, income classes and index bases stay soft (the real #140 class).
