...
3. **Usage-based subscription (Stripe metered billing).** Contradicts the decided model (no subscription) — rejected; revisit only if the newsroom tier wants it. **Superseded for the Pro tier specifically, 2026-09-14 (open-questions [#205](../open-questions.md)):** metered/usage-based billing is still rejected in the form above, but a real recurring subscription now exists alongside one-time credit packs — a flat €19.99/month Stripe subscription Checkout (`mode: 'subscription'`) granting a fixed monthly credit allowance, not a usage-metered charge. One-time credit packs remain the default, unaffected purchase path; the subscription is an additive second option, shipped fully built but flag-gated behind `PRO_SUBSCRIPTIONS_ENABLED` (off until the owner explicitly flips it — see [docs/RUNBOOK.md](../RUNBOOK.md)'s Pro subscription go-live section). Full design: [docs/superpowers/specs/2026-09-13-pro-subscription-tier-design.md](../superpowers/specs/2026-09-13-pro-subscription-tier-design.md); implementation plan: [docs/superpowers/plans/2026-09-13-pro-subscription-tier.md](../superpowers/plans/2026-09-13-pro-subscription-tier.md). ADR [020](020-credit-ledger-and-billing-gate.md)'s decision 8 carries the matching revision note for the Stripe/ledger mechanics.
