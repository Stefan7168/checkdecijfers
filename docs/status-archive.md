...
**End state, this stretch:** PRs #30 and #31 **MERGED** (both squash commits CI-green on `main`); PRs #23, #24, #25, #26, #27, #28, #29 **still open**, untouched by this stretch, unaffected by either merge (confirmed no file overlap). See Lessons for the two process lessons this stretch surfaced (subagents cannot "wait" on their own background processes the way the main session can; `git merge-tree`'s base argument must be the real merge-base, not current `main`).
