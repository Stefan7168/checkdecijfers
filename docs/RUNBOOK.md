...
- [ ] **Transactional email provider: Resend** (decided 2026-07-04, ADR [006](decisions/006-auth-billing-seams.md) — custom SMTP for Supabase Auth's magic-link emails; Supabase's own built-in sender is dev/test-only and rate-limited) — also carries your owner alerts (ingestion failures, quarantines, missed syncs). Once created: Supabase dashboard → Authentication → Emails → SMTP Settings, enter the Resend SMTP credentials.
- [ ] **Supabase Auth → URL Configuration**: set Site URL and add Redirect URLs for both `http://localhost:3000/auth/callback` (local dev) and the deployed app's `https://.../auth/callback` — required for the magic-link email's link to actually land back in the app (`web/app/auth/callback/route.ts`).
- [ ] **Stripe test-mode account** — free, instant, no KvK needed for test mode. Once created: Dashboard → Developers → API keys (test mode) for `STRIPE_SECRET_KEY`; Dashboard → Developers → Webhooks → add endpoint (`.../api/stripe/webhook`) for `STRIPE_WEBHOOK_SECRET`. Also enable iDEAL under Settings → Payment methods (test mode) so it appears in Checkout's dynamic payment-method selection (`src/billing/stripe-checkout.ts` deliberately never hardcodes payment methods).
...
- [ ] **KvK registration + business bank account** — required before Stripe can pay out in the Netherlands (real money; test mode above needs none of this). Start early; this is the slowest item on this list, and its timeline is still undecided ([open-questions #54](open-questions.md)).
...
| `NEXT_PUBLIC_SUPABASE_URL` | `web/.env.local` + Vercel env store (**not yet set — WP13, 2026-07-04**) | Not secret (public project URL) — Supabase dashboard → Project Settings → API → Project URL. `NEXT_PUBLIC_` vars are baked into the client bundle by Next.js at build time; that's expected here |
| `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` | `web/.env.local` + Vercel env store (**not yet set — WP13, 2026-07-04**) | Not secret (Supabase's current name for what used to be called the "anon key" — rate-limited/RLS-scoped by design) — Supabase dashboard → Project Settings → API → Publishable key |
| `STRIPE_SECRET_KEY` | `web/.env.local` + Vercel env store (**not yet set — WP13, 2026-07-04, awaiting the Stripe test-mode account above**) | Real secret. Stripe Dashboard (test mode) → Developers → API keys → create/roll a **restricted key** (prefer over the full secret key — least privilege) → replace in both places → delete the old key in the Stripe dashboard |
| `STRIPE_WEBHOOK_SECRET` | `web/.env.local` + Vercel env store (**not yet set — WP13, 2026-07-04**) | Real secret. Stripe Dashboard → Developers → Webhooks → your endpoint → "Signing secret" → reveal → replace in both places. Rotating it requires updating the webhook endpoint's signing secret in the same dashboard screen |
| `NEXT_PUBLIC_APP_URL` | `web/.env.local` + Vercel env store (**not yet set — WP13, 2026-07-04**) | Not secret — the deployed app's own origin (e.g. `https://checkdecijfers.vercel.app`, or `http://localhost:3000` locally), used to build the magic-link redirect and Stripe Checkout success/cancel URLs (`web/app/login/actions.ts`, `web/app/credits/actions.ts`) |
