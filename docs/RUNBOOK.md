...
|---|---|---|
| GitHub | `Stefan7168` / stefanpeek00@gmail.com | repo owner; other gh accounts 404 on the private repo |
| Anthropic (API) | stefanpeek00@gmail.com | console.anthropic.com; €25/mo cap + alert set |
| Supabase | stefanpeek00@gmail.com, org **"stefan"** | ⚠ a second empty org "Stefan7324" exists on this login AND a second Supabase account (glaibaan) is often the browser's active session — always confirm org "stefan" |
| Vercel | stefanpeek01@gmail.com | **different email from the others**; project `checkdecijfers` |
...
1. **Install** (one-time): [Node.js](https://nodejs.org) **24 or newer**
   ...
   **Vercel CLI** (`npm i -g vercel`, then `vercel login` as
   stefanpeek01@gmail.com and `vercel link` to project `checkdecijfers`) —
...
2. **Sign in**: GitHub (your `Stefan7168` account — `gh auth login` walks you
   ...
   **Multi-account gotchas (previously only in AI session memory — recorded
   ...
   - **GitHub:** the machine may hold several gh accounts (work ones like
     `socialplus-tools`). The repo is private under `Stefan7168`; any other
