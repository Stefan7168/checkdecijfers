...
3. **Verify the cron fires**: Vercel dashboard → the project → Settings → Cron Jobs should show `/api/onboarding-cron` every 2 minutes (it comes from the committed `web/vercel.json`); its invocations should return 200 with `{"claimed":0}`-style output once the secret is set (503/401 before that is the fail-closed design, not a bug).
