...
1. **Google Cloud Console** (free; use your PERSONAL Google account — the Chrome profile named 'Personal', owner steer 2026-07-10 — so the OAuth project lands under the right identity): create a project → "OAuth consent screen" (External, app name checkdecijfers, your e-mail) → "Credentials" → "Create credentials → OAuth client ID" → type **Web application** → Authorized redirect URI: `https://<project-ref>.supabase.co/auth/v1/callback` (the exact value is shown in step 2's Supabase screen — copy it from there). Save the **Client ID** and **Client secret**.
