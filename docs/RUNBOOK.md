...
- **Monthly maintenance session** — start a chat with: *"Run the monthly maintenance session for checkdecijfers.nl per CLAUDE.md."* Agenda: dependency alerts, provider deprecation notices, spend dashboards, backup status.
  - **⚠ The GDPR purge belongs on this agenda and nothing schedules it — see [#189](open-questions.md)**, recorded the same night by the parallel session, whose RUNBOOK edit adds the agenda line itself. Two operational notes that belong with it: **the 90-day leg is the one with a real deadline** — the first live anonymous-trial bookkeeping rows were written at the #53 go-live on **2026-07-17**, so they cross their window around **2026-10-15**, and from then on a month without a run means `trial_questions` is retaining visitor ids + IP hashes past their declared purpose (ADR 036 D4); the 2-year audit leg has no live deadline yet. And **if a run prints `note: trial_questions absent (migration 020 not applied)` against production, that is now a REAL signal, not a shrug** — migration 020 has been live since 2026-07-17, so investigate rather than continue.
