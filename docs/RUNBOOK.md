...
| `CLARIFY_CLICK_ENABLED` | Vercel env store only (**✅ SET `1`, Production, 2026-09-02, session 69, owner present**) | Not secret — the literal value `1`. WP26 mechanism A: clarification chips carry a pre-verified intent, a byte-equal reply is taken without an LLM; #197 comparison chips under answers ride the same take-path. Smoke-tested 2026-09-03 (audit rows 261/262, 263/264). Rollback: remove + redeploy — but read the ROLLBACK ORDER in the WP26 section first (this flag goes off FIRST, `ANSWER_FIRST_ENABLED` a day later). |
