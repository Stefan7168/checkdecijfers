...
- [x] **Anthropic API** — account created 2026-07-02 (personal-gmail-1-redacted); key stored in local `.env`; €25 monthly spend cap confirmed set by owner (2026-07-02). ⚠ **One owner step open (WP11, 2026-07-03): set a billing alert** so you get an *email before* the cap bites (the cap silently blocks API calls when hit; an alert warns you first). Steps: log in at [console.anthropic.com](https://console.anthropic.com) → Settings → Billing / Usage limits → set an email notification threshold well under the cap (suggestion: ~€10/month — normal build-phase sessions spend well under €2). The exact menu label may differ slightly as the Console evolves; you're looking for "notification" or "alert" next to where the monthly spend cap was set. A session cannot do this for you — it's inside your Console login. When done, tell a session so STATUS gets ticked.
