...
| `ONBOARDING_OFFER_SECRET` | Nowhere yet (**NOT SET — the #109 confirm-first reversal, built 2026-09-13, session 101, branch `journey-r3-fetch-confirm`, not yet merged; owner sets it before/at merge or the feature degrades honestly, see below**) | Real secret you invent yourself (password-manager generator, or `openssl rand -base64 32`). HMAC-signs/verifies the "confirm this 100-credit fetch?" offer (`src/ingestion/onboarding-offer-token.ts`) — a stateless token, same shape as `EMBED_TOKEN_SECRET`, no database row per offer. **Missing → fails closed, same posture as `EMBED_TOKEN_SECRET`:** the confident-finder acknowledgment shows an honest "not available right now" instead of a confirm button, and nothing is charged — never a broken button, never a silent auto-fetch reverting to the pre-#109 behavior. **Rotation is benign, unlike `EMBED_TOKEN_SECRET`:** an offer is only ever meant to live a few minutes (30-minute token expiry) — replacing this value just invalidates any offer a reader is mid-decision on right now, who then simply asks again. No stored state to reconcile either way. **Requires `ONBOARDING_ENABLED=1` to matter at all** (this only gates what happens once that flow already fires) |
