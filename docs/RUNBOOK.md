...
2. **Set `CRON_SECRET` and `RESEND_API_KEY`** in the Vercel env store (rows in the secrets register above) and **redeploy** (env edits never apply to a running deployment).
