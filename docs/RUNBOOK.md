...
4. Full verification block + `/code-review` low + push; after green CI the live step: `node --import ./scripts/force-ipv4.mjs --env-file=.env src/ingestion/cli.ts sync <id>` (auto-registers) + `npm run registry:apply`, then an LLM-free `runQuery` spot-check of the frozen-key cell.
