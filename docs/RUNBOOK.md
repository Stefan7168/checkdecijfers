...
| `NEXT_PUBLIC_SUPABASE_URL` | `web/.env.local` + Vercel env store — **set 2026-07-04** | Not secret (public project URL) — Supabase dashboard → Project Settings → API Keys → Project URL. `NEXT_PUBLIC_` vars are baked into the client bundle by Next.js at build time; that's expected here |
| `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` | `web/.env.local` + Vercel env store — **set 2026-07-04** | Not secret (Supabase's current name for what used to be called the "anon key" — rate-limited/RLS-scoped by design) — Supabase dashboard → Project Settings → API Keys → Publishable key. ⚠ The same page also shows **Secret keys** — never navigate/screenshot that page as a whole; copy only the publishable key value |
...
| `NEXT_PUBLIC_APP_URL` | `web/.env.local` (`http://localhost:3000`) + Vercel env store (`https://checkdecijfers.vercel.app`) — **set 2026-07-04** | Not secret — the deployed app's own origin, used to build the magic-link redirect and Stripe Checkout success/cancel URLs (`web/app/login/actions.ts`, `web/app/credits/actions.ts`) |
