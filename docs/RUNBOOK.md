...
- [x] **Anthropic API** — account created 2026-07-02 (personal-gmail-1-redacted); key stored in local `.env`; €25 monthly spend cap confirmed set by owner (2026-07-02).
- [x] **Supabase** — account created 2026-07-02 (personal-gmail-1-redacted). Nothing vendor-specific will be used (ADR [002](decisions/002-postgres-system-of-record.md)), so this stays swappable. `DATABASE_URL` stored in local `.env` (2026-07-02); its first verified use comes with the ingestion session. ⚠ **2026-07-03 fix:** the project's "Automatically expose new tables" API setting was on, granting `anon`/`authenticated` full CRUD on every table via Supabase's auto-generated Data API — this app never uses that API (ADR 002/003: server-side `pg` over `DATABASE_URL` only), so it was pure unneeded attack surface. In practice not exploitable (Supabase's own RLS-auto-enable safety net had already blocked real access), but fixed properly anyway: migration `003_lock_down_api_access.sql` revokes those grants and locks default privileges for future tables. **One step only you can do:** in the Supabase dashboard, go to **Settings → API → Data API** and turn **off "Automatically expose new tables."** The migration protects tables our own code creates; only that dashboard toggle stops Supabase's platform automation from re-exposing a brand-new table the same way.
