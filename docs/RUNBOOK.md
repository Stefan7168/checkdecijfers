...
| `CLARIFY_CLICK_ENABLED` | Vercel env store only (**✅ SET `1`, Production, 2026-09-02, session 69, owner present**) | Not secret — the literal value `1`. WP26 mechanism A: clarification chips carry a pre-verified intent, a byte-equal reply is taken without an LLM; #197 comparison chips under answers ride the same take-path, and since #73 v2 (session 72, PR pending owner review) so does every follow-up chip under an answer. Smoke-tested 2026-09-03 (audit rows 261/262, 263/264). Rollback: remove + redeploy — but read the ROLLBACK ORDER in the WP26 section first (this flag goes off FIRST, `ANSWER_FIRST_ENABLED` a day later). |
...
6. **#197 step 3 — ✅ RUN 2026-09-03 (session 71, owner): rows 263 (the Amsterdam answer, chip minted) and 264
   ...
   until the tab reloads — one ordinary charge, never a wrong number. **#73 v2 (session 72, 2026-09-03, PR
   pending owner review): every chip under an answer — the adjacent-period, trend, region-variant and same-topic
