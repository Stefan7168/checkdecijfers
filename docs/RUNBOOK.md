...
- [x] **Anthropic API** — account created 2026-07-02 (stefanpeek00@gmail.com); key stored in local `.env`. ⚠ Still confirm in console.anthropic.com that a **monthly spend cap** is set (suggestion: €25; Phase 0 usage is cents per benchmark run).
- [x] **Supabase** — account created 2026-07-02 (stefanpeek00@gmail.com). Nothing vendor-specific will be used (ADR [002](decisions/002-postgres-system-of-record.md)), so this stays swappable. ⚠ Still to do: paste the connection string into `.env` as `DATABASE_URL` (dashboard → Connect → URI).
