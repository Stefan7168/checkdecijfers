...
- [x] **Supabase** — account created 2026-07-02 (personal-gmail-1-redacted). Nothing vendor-specific will be used (ADR [002](decisions/002-postgres-system-of-record.md)), so this stays swappable. `DATABASE_URL` stored in local `.env` (2026-07-02); its first verified use comes with the ingestion session.
