...
- **Monthly maintenance session** — start a chat with: *"Run the monthly maintenance session for checkdecijfers.nl per CLAUDE.md."* Agenda: dependency alerts, provider deprecation notices, spend dashboards, backup status, **and `npm run gdpr:purge` (dry-run first, then `-- --apply`)**.
  - ⚠ **The purge is the ONLY thing that enforces retention — nothing schedules it** ([#189](open-questions.md), found 2026-07-25). There is no cron and no CI schedule; `web/vercel.json` carries one cron and it is the onboarding job. Two clocks run off this one command: audit rows redact at 2 years, and `trial_questions` bookkeeping (visitor UUID + HMAC'd IP) DELETES at 90 days. The first trial rows become purgeable **~2026-10-15**; if nobody runs this, they simply stay. Whether it becomes a cron is an open owner decision.
