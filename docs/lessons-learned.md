...
### Thread A (owner-present, this session): PR #9's fifth/sixth conflicts + five experience-improvement-plan
quick wins — an unmocked server action's real jsdom behavior wasn't what a comment assumed, and three
numeric assumptions in money-adjacent UI code were quietly wrong until an adversarial review pass caught
them
...
### Thread B (a parallel session, same day): strategy / research session, docs only
...
### Thread A/B collision: two sessions can independently build/plan the SAME work, unaware of each other,
because neither reads the other's in-flight changes until a `git fetch` forces the question

Thread A built exactly what Thread B's Journey-programme Phase 1 (R2+R10+R11) independently specified as
"NEXT UP" — same report, same recommendations, arrived at separately within hours of each other, on
different branches (`claude/checkdecijfers-embed-pr-review-acbrd5` vs. direct pushes to `main`). No time or
correctness was actually lost (Thread A's work and Thread B's plan turned out to describe the identical
scope, and reconciling was a mechanical docs-numbering fix, [#239](open-questions.md)) — but it was luck,
not design: a `mergeable_state` flip from clean to dirty (a routine PR check-in) was the only thing that
surfaced Thread B's existence to Thread A at all. Lesson: a long-running session on a feature branch should
re-`git fetch origin main` and skim the new commits' subjects **before** starting a new chunk of autonomous
work, not only when a stale-mergeable-state notification forces the question — a two-line `git log
HEAD..origin/main --oneline` check is cheap insurance against building (or worse, deciding) the same thing
twice. When it does happen anyway, the fix is the ordinary renumbering playbook already used all session for
open-questions collisions, not a redo of either side's work.
