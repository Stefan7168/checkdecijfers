...
**Adds:** credit packs via Stripe + iDEAL on the append-only ledger (ADR 006); chat behind account creation with free signup credits (decided 2026-07-02; amount and cadence: [open-questions.md](open-questions.md) #3); **full browse layer** — complete page inventory plus the chat-teaser signup flow (parsed question shown, validated answer after signup); pre-spend cost transparency (estimate → confirm; clarifying questions cost 0; credit-cost labels on drill-down buttons); **shareable answer pages with OpenGraph chart images** (Stefan's programmatic-SEO idea — pages render from audit records; static-image renderer over the chart spec, ADR [007](decisions/007-chart-spec-rendering.md)); watermark/"Gegenereerd via checkdecijfers.nl" on shared visuals; user-facing audit-trail view (the answer's source appendix); payments GDPR items; public-launch rate limiting (ADR [005](decisions/005-caching-strategy.md) trigger); daily Stripe-vs-ledger reconciliation with owner alert on mismatch (+ webhook-failure alerting); a support channel with a stated response time; the published-page correction policy implemented ([open-questions.md](open-questions.md) #22).
