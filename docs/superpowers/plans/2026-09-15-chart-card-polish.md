...
- [x] **Step 1: Update the pin first (this is a deliberate default change, the ADR 042 precedent):** in `chart-presentation.test.ts:94` change `framePadding: 'none'` to `framePadding: 'small'` inside the STOCK expectation and rename the test to `'STOCK_PRESENTATION is the designed default: ends markers, horizontal grid, no axis lines, gradient area fill, rounded soft-shadow frame with small padding'`. Grep `web/` for `framePadding: 'none'` in test files and update ONLY the pins that literally encode the stock default (a fixture that tests `isFramePristine`'s bare state stays `'none'` — that is the mechanism, not the default). **As-built (2026-09-16, session 106):** one further test needed the same treatment beyond what this step anticipated — `chart-config-panel.test.tsx`'s "Kader wissen is disabled at the literal bare frame values" test relied on STOCK's `framePadding` already being `'none'` to reach the bare/`isFramePristine` state; it now overrides `framePadding: 'none'` explicitly alongside `frameCorners`/`frameShadow`, same pattern as the `b86556f` precedent it already documented in its own comment.
- [x] **Step 2: Run** `cd web && npx vitest run lib/chart-presentation.test.ts lib/chart-templates.test.ts` → the STOCK pin FAILS.
- [x] **Step 3: Implement** — `chart-presentation.ts:136`: `framePadding: 'small',` with the comment on the constant extended: `// 2026-09-15 (chart-card polish, owner-confirmed): small frame padding so the plot no longer sits flush against the rounded, shadowed frame edge b86556f introduced.`
- [x] **Step 4: Run** `cd web && npx vitest run` (the whole web suite — a padding default touches ChartFrame's `naturalHeight`, the export canvas and the templates' contrast gate) → PASS after the Step-1 pins. **As-built: 1817/1817 web tests pass** (measured 2026-09-16; the `app/bevolking-3d-demo/*.test.ts(x)` suite's `three` import failures seen on a first run were a pre-existing stale local `node_modules` gap — `three` is a real `package.json` dependency, not installed locally — fixed with a plain `npm install`, unrelated to this change and confirmed via `git status` that no lockfile moved).
- [x] **Step 5: Typecheck** — `cd web && npx tsc --noEmit`. **As-built: clean**, and the repo-root `npm run typecheck` (backend) also clean.
- [x] **Step 6: Commit** — as part of session 106's push (owner-present, direct to `main` per #118(a); see the session's own commit for the exact message and SHA).

**Owner decision, 2026-09-16 (session 106, chat):** triggered not by a fresh before/after review of Tasks 1-4 (those already shipped 2026-09-15), but by comparing this product's chart polish against Supademo's report charts (`supademo.com/content/state-of-interactive-demos-2026`) — see [open-questions #260](../../open-questions.md) for the fuller Supademo-comparison context this sits inside. The owner was shown the live Style panel toggling `framePadding` between `none` and `small` on a real gallery-story chart, plus the plain-language trade-off (a little breathing room inside the rounded/shadowed frame vs. staying flush), and chose small padding as the new default over keeping it flush or reviewing medium/large first.
