...
> **Note added session 120 (2026-09-20), [#304](../../open-questions.md):** the blanket "pie, stacked, scatter and ranked charts are never offered" framing below (in the Goal, the Global Constraints, and the Task 5 commit message near line 66) was accurate when this plan was written and executed, but is now **PARTIALLY SUPERSEDED** by chart co-pilot phase 5b (session 118, merge `c7723c34`): pie, stacked, and 100%-stacked are now *conditionally* offered on CBS/Eurostat charts whose regions form a complete, verified roster — see ADR [056](../../decisions/056-chart-copilot.md) phase 5b and the phase 5b row in [docs/04-architecture.md](../../04-architecture.md). Scatter and sorted/ranked bars (and smoothed/step curves) are still never offered. This file is left otherwise unchanged as a historical record of what phase 5 itself shipped.
...
- **Refused for every dataset, never rendered:** pie/donut, stacked bar/area, 100 %, scatter, sorted/ranked bars, smoothed/step curves. **Superseded (noted session 128, 2026-09-25, [open-questions #304](../../open-questions.md)):** since chart co-pilot phase 5b (session 118, ADR [056](../../decisions/056-chart-copilot.md)) pie/donut, stacked and 100 %-stacked ARE offered, conditionally (a verified-whole region roster only); scatter is still refused ([#296](../../open-questions.md)). This line records the phase-5 plan as written. The panel's Grafiek tab gets ONE collapsed note (`<details>`) `Waarom geen taart- of gestapelde grafiek?` with three digit-free sentences (a pie or a stack draws a total or a share no CBS cell backs; a scatter needs two measures; sorting is a ranking claim), nl + en.
