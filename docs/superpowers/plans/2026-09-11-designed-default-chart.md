...
- **Tooltip:** ~~a muted dashed crosshair on line/area~~ — **superseded during the build (Task 3 review ruling, ADR 042 decision 10): a faint SOLID hairline (`strokeWidth 1`, `strokeOpacity 0.35`), never dashed — the `3 3` dash is byte-identical to the curated event marker and close to the dashed story ring, and on a touch device the cursor persists after a tap** — a muted band on bars; the export drops any `.recharts-tooltip-cursor` node (and, after the whole-branch review, any `.recharts-active-dot`) so a touch device's active cursor can never bake into a PNG as a fake annotation line.
