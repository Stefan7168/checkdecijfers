...
- Out of scope, left byte-identical in both languages: everything the backend builds — answer text, refusals, clarification chips, attribution/definition/staleness lines, question history text, source names (`src/answer/*`, `src/billing/history.ts`, `src/sources/registry.ts`); the attachments backend's own English templates (`src/attachments/templates.ts`, ADR 037). **Superseded 2026-09-17, session 110 (UX audit row 21):** the site-wide footer's `FOOTER_ATTRIBUTION` sentence (`web/components/site-footer.tsx`) was originally pinned byte-identical Dutch here too — an English page showing Dutch attribution copy read as unfinished, so it now translates through `messages.ts` (`footer.attribution`) like the rest of the interface catalogue. "CBS StatLine (CC BY 4.0)" (R4 attribution) stays verbatim in both languages, asserted in `web/lib/i18n/messages.test.ts`. This does not touch the backend-built attribution/definition/staleness lines listed above, which stay out of scope and Dutch.
...
7. **Dates:** `nl-NL`/`en-GB` for the two `Intl` call sites that format for display (question history, citation). The `en-CA` YYYY-MM-DD trick stays (not a language). **As built, session 91: only the question-history date switches locale. The citation date stays `nl-NL` always — the citation text as a whole stays Dutch on purpose, because it quotes a Dutch answer sentence, so translating just its date while the sentence around it stays Dutch would be inconsistent.**
