...
| 5 | *"Vergelijk de bevolkingsgroei van de vier grote steden"* | **Still refuses** — `growth` is two periods and the G4 is four regions, and ADR 011's one-varying-axis rule refuses "several regions AND several periods" (`src/query/resolve.ts:289-290`). This spec does NOT lift that rule. The G4 *level* question ("hoeveel inwoners hebben de vier grote steden?") already works today: the prompt teaches the G4 as four named gemeenten (`src/answer/intent/prompt.ts`, Regions rule 3). **Superseded (session 110, same day, ADR [055](../../decisions/055-multi-region-series.md)): a closely related question — the G4's own multi-year TREND, e.g. "Hoe ontwikkelde de bevolking van de vier grote steden zich sinds 2015?" — is now answered as a `'region_series'` result (one line per city). This row's own `growth`-derivation phrasing (`difference` over exactly 2 periods) still refuses exactly as stated; only the `series`-shaped trend case was lifted, by a different ADR, out of this spec's own scope.** |
