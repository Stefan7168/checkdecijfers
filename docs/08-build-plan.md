...
**Execute-ready.** The #123 source choice is made: Eurostat. ADR 048 is the full brief (Decision D1–D10,
...
## Chart alternate-reading toggle — session 106 (2026-09-16), PAUSED mid-build, NOT merged

**[#254](open-questions.md)'s second gap (context controls) → design corrected once against the real registry (20 measures carry `alternates`, not 1 as first assumed) → 7-task plan → subagent-driven development in worktree `worktree-chart-alternate-reading-toggle`.** Spec:
[superpowers/specs/2026-09-16-chart-alternate-reading-toggle-design.md](superpowers/specs/2026-09-16-chart-alternate-reading-toggle-design.md). Plan:
[superpowers/plans/2026-09-16-chart-alternate-reading-toggle.md](superpowers/plans/2026-09-16-chart-alternate-reading-toggle.md). Full record: [open-questions #261](open-questions.md), `status-archive.md`'s session-106 entry.

Built: a shared `buildAlternateReading` function (`src/chart/alternate-reading.ts`, generalizing `curated.ts`'s narrower one, merging dims over the primary's resolved coordinates rather than replacing them, with an enforced period-window-match guard added mid-review), a new additive `AnswerResponse.chartAlternates` field (never inside `ChartSpec`), `selectedReading` view state + `activeReadingSpec` in `chart-view-state.ts`, and a reading-select dropdown in `ChartView` built specifically to never trip the component's existing spec-identity-reset effect (dispatched on the most capable model tier for both implementer and reviewer, given the stakes; reviewer confirmed zero Critical findings and independently verified the reset effect was never touched). 5 of 6 plan tasks + one standalone mid-build fix are complete and review-clean.

**PAUSED, not finished: Task 6 (the final wiring that activates the toggle in `chat.tsx`/`visual-dock.tsx`) has one open, unfixed review finding** (`trial-chat.tsx`, the anonymous homepage trial, silently missing the toggle — not in Task 6's file list), and Task 7 (the whole-branch review) has not run. Branch pushed to `origin/worktree-chart-alternate-reading-toggle` as a durability backup (no PR). **Next session: resume this via `EnterWorktree` at the existing local path if available, else check out fresh from the pushed branch — do not start over.** Resolve Task 6's finding, run Task 7, then the full verification block + `/code-review` LOW + direct merge to `main` (owner-present convention) + push + confirm CI green.
