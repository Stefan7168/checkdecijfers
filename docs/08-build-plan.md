...
## WP30 — multi-source architecture: formalize the narrow waist (priority #2, designed 2026-07-08)  ✅ WP30a MERGED + DEPLOYED — LIVE (session 36, 2026-07-11, PR #26, merge `7864271`); ✅ WP30b MERGED + DEPLOYED — LIVE (session 37, 2026-07-12, PR #27, merge `f6bcf26`, main gate + deploy green); ✅ WP30c E1 BUILT (session 101 continuation, autonomous, 2026-09-14/15) — branch + PR pending owner review, not merged — see the WP30c entry below
...
PR pending, NOT merged (autonomous, core-product code, #118(b)).** Frozen executor brief
...
Amendments B1–B6), then built. **Read ADR 048's new "As-built" section for the full account** — headline: a
new session-level scoping decision ("Constraint 0") meant NO live Eurostat API call happened this session, so
this build's done-definition is narrower than the one below (everything buildable without a live call was
built and verified; real fixture capture / the Amendment-12 smoke probe / "≥3 real datasets rendered" are the
one clearly-scoped owner-run follow-up, [#249](open-questions.md)). A real chip-visibility gap this build
found (not anticipated by either adversarial review round) is also fixed: registering the Eurostat source
alone would have surfaced a live "Eurostat data" chip to real users — see ADR 048's as-built note. Full
verification: root + web typecheck clean; backend suite 160 files / 2426 tests green (solo); web suite 106
files / 1743 tests green (solo); hermetic benchmark 14/14 + 6/6 + 0 fabricated (byte-identical, zero prompt
bytes touched); a real `next build` succeeds. The original entry below is kept as the design record; it no
