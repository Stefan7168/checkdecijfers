...
## WP12 — Minimal chat UI + first deploy  ← NEXT

*Added 2026-07-03 (WP11 session): this entry closes a plan gap — [03-mvp-scope.md](03-mvp-scope.md) lists "Chat UI, minimal" as Phase 0 in-scope, and three earlier decisions defer work "to the chat-UI session" — but the plan previously ended at WP11.*

**Scope:** the docs/03 minimal chat UI (one conversation, no persistence beyond the session; ugly is acceptable) over the existing audited entry points (`answerQuestionAudited` / `answerClarificationReplyAudited` — the only calls the UI makes; ADR [016](decisions/016-audit-records.md)), on the ADR [008](decisions/008-ui-foundation.md) foundation (Next.js on Vercel). Deferred obligations that land here: the Recharts client wrapper over ChartSpec v1 (ADR [014](decisions/014-chart-spec-v1-and-renderer.md) deviation), Vercel CLI login + env store setup with the owner ([RUNBOOK.md](RUNBOOK.md) secrets register), and **deploy-blocking CI** (docs/03 ops guardrails; the CI workflow note "deploy-blocking attaches at Vercel setup"). Invariants at stake: the UI must never bypass the audited entry points (R8), never render an unvalidated number (R1–R3), and the one-round clarification state must flow through `PendingClarification` untouched (ADR 015).

**Done means:** the 20 benchmark questions can be asked through the deployed UI by hand and produce the same audited outcomes the WP11 live run recorded; a red CI blocks the deploy; STATUS updated with the deploy URL and any deviations recorded in an ADR. Phase-gate sign-off (owner) can happen before or in parallel — the gate is decided on the WP11 scoreboard row, not on UI polish.
