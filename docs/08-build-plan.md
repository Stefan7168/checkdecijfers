...
## Chart co-pilot — chat and direct controls interleaved on one chart (owner decision 2026-09-17/18, session 111; ADR [056](decisions/056-chart-copilot.md), spec [superpowers/specs/2026-09-17-chart-copilot-design.md](superpowers/specs/2026-09-17-chart-copilot-design.md)) — phases 1 + 2 + 3 + 4 + 5 BUILT (sessions 112/113/114/115/116); phase 5b — the "verified whole" (spec §11) — IN PROGRESS, 4/5 tasks built on branch `verified-whole-phase5b` (session 117, not yet merged; see [STATUS.md](STATUS.md)); the six house styles + homepage themes row ([#275](open-questions.md)) remains the next NOT-yet-started priority once phase 5b lands
...
6. **Phase 5b — the "verified whole": pie, stacked, 100%-stacked — IN PROGRESS (session 117, 2026-09-19/20, 4 of 5 tasks via subagent-driven development, plan [superpowers/plans/2026-09-19-verified-whole-phase5b.md](superpowers/plans/2026-09-19-verified-whole-phase5b.md), NOT YET MERGED to `main` — branch `verified-whole-phase5b`).** Picks up the two capabilities phase 5 split out. Region hierarchies only (provinces/landsdelen/gemeenten-of-a-province — the one place CBS already vouches for a complete set); verification on demand, server-side, the same "Option A" pattern as phase 4's difference/average, no new CBS fetch. Donut stays a presentation variant of pie. Scatter remains untouched, still deferred. **Task 4 (the on-demand verification server action + Recharts rendering) has 2 open Important review findings pending a fix round; Task 5 and the final whole-branch review have not started.** See ADR 056 §"Phase 5b — in progress" and the SDD ledger (`.superpowers/sdd/2026-09-19-verified-whole-phase5b/progress.md`) for the exact resume state.
