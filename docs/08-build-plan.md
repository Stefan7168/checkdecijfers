...
**Session 101 continued (2026-09-13, owner present) — phase 2 / R3 BUILT on branch `journey-r3-fetch-confirm`, PR pending (not merged — branch + PR + owner review required for this one even though the owner is present in the session, per the rule below).** The confirm-first chip before the 100-credit on-demand fetch (#109's reversal, decisions 4-5 answered directly in chat — see [open-questions #109](open-questions.md)) and its full mechanism (a signed, stateless offer token, no database change) are recorded as-built in [ADR 026](decisions/026-on-demand-fetch-job-architecture.md)'s addendum. New required secret, not yet set anywhere: `ONBOARDING_OFFER_SECRET` (RUNBOOK Secrets register) — until the owner sets it, the feature fails closed to an honest "not available right now" message, never a silent revert to the pre-#109 automatic-charge behavior. Decision 5 (larger signup grant) was answered **no** — no config/schema change. Full verification (typecheck ×2, backend + web suites, hermetic benchmark, real build, new token tests, new chat UI tests) before the PR opens.
