...
| `cbs_catalog` mirror + `src/catalog/` table finder | Demand-driven discovery (WP16 sub-part 1): finds which CBS table answers a not-yet-loaded topic — Dutch FTS recall + a rerank picking from a hard-allowlisted shortlist, over our own mirrored metadata, never a live CBS call in the request path. Built and live-calibrated; **wired into the answer flow and LIVE in production since the session-28 go-live (2026-07-06, `ONBOARDING_ENABLED=1`)** — see [STATUS.md](STATUS.md). **Since WP27 stage A ([PR #17](https://github.com/Stefan7168/checkdecijfers/pull/17), awaiting merge): question-aware rerank (full question + topic, prompt v2 incl. the vague-question honesty rule) + Regulier-first shortlist quotas (20 current + 4 historic)** | [025](decisions/025-cbs-catalog-table-discovery.md), [026](decisions/026-on-demand-fetch-job-architecture.md), [027](decisions/027-finder-shape-fit-gate.md) |
