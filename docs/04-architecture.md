...
| Demand-driven table onboarding (user-triggered fetch of not-yet-loaded tables) | **Discovery half BUILT (WP16 sub-part 1, ADR [025](decisions/025-cbs-catalog-table-discovery.md)):** `findTable` over the `cbs_catalog` mirror, live-calibrated, awaiting sub-part 2 wiring. Fetch half: same `CbsSource` adapter + ingestion/validation pipeline, triggered by a user action instead of a schedule; registry `needs_review` state doubles as the review queue; size preflight rides the cost-estimation step (ADR 006) |
