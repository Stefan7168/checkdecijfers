...
| Unit-expansion derivation + display splice (`deriveUnitExpansion`, `src/answer/compose/expand.ts`) | **BUILT (#125a, session 36, 2026-07-11; PR pending owner review).** Pure numeric factor units (`x 1 000`) pre-register the exact expanded figure as a `unit_expansion` DerivationRecord (integer-scaled arithmetic, never float; refusal = fail-open) and a deterministic post-validation splice shows it alongside CBS's verbatim notation — `390,2 x 1000 (= 390.200)`. The LLM never sees the record (payload filter keeps every fixture byte-identical) and never renders it; the spliced body is what R8 stores; old audit rows are structurally out of reach | [031](decisions/031-unit-expansion-derivation.md) |
