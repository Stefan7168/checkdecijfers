...
// Session 128 (chart.tsx split, docs/decisions/ ADR — module boundaries
// unchanged, ADR 001): chart.tsx's own module-level pure types/constants/
// builder functions and its render-helper components used to be defined
// right here, inline. They now live in two sibling files — pure, React-free
// models in `../lib/chart-models.ts`, JSX render helpers (tooltips, axis
// ticks, overlays, bar/dot shapes) in `./chart-parts.tsx` — moved verbatim,
// same names/bodies/comments, no behaviour change. Every symbol this file
// exported before the split is re-exported below (`export { ... }` /
// `export type { ... }`), so no importer anywhere needed to change.
import {
  annotationMarkers,
  AXIS_COLOR,
  baselineAxisLine,
  ...
  clampTotChange,
  clampVanafChange,
  derivationRefusalMessage,
  embedHeightValue,
  EMBED_ATTRIBUTION_RESERVE_PX,
  GRID_COLOR,
  GRID_LINE_PROPS,
  heatmapIntensity,
  heatmapModel,
  humanizeNullNote,
  KNOWN_DERIVATION_REFUSAL_KEYS,
  labelWidthPx,
  seriesStyle,
  tableModel,
  translateSpecForDisplay,
  valueLabelPlan,
  wholeRefusalKey,
  yAxisDomain,
  type AxisTickLabel,
...
import {
  ...
  StageLegend,
  useCoarsePointer,
  type DumbbellChartRow,
  type EndLabelSpec,
} from './chart-parts.tsx';
...
export { RECHARTS_PALETTE, DEFAULT_PALETTE } from '../lib/chart-models.ts';
