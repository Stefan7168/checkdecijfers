...
describe('resolveDerivedOverlays', () => {
  it('resolves every pending request and keys the results by overlay id', async () => {
    ...
    const { resolvedMap } = await resolveDerivedOverlays(
      [{ id: 'd1', calcKind: 'difference', resultIds: ['r1', 'r2'] }],
      requester,
    );
    expect(requester).toHaveBeenCalledWith('difference', ['r1', 'r2']);
    expect(resolvedMap.get('d1')).toEqual({ kind: 'difference', value: 5, sourceResultIds: ['r1', 'r2'], unit: '', marking: 'CC BY', explicit: true, minuendResultId: 'r2', subtrahendResultId: 'r1' });
  });
  ...
  it('a refused request is simply absent from the result map, never a thrown error', async () => {
    ...
    const { resolvedMap, refusals } = await resolveDerivedOverlays([{ id: 'd1', calcKind: 'mean', resultIds: ['r1', 'r2'] }], requester);
    expect(resolvedMap.has('d1')).toBe(false);
    expect(refusals.get('d1')).toBe('nope');
  });
});
