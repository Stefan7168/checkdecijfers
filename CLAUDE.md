...
**GOLDEN RULE — verify every fact against reality, never from memory** (added session 44, 2026-07-16, after a wrong date was written into every session-44 doc from recollection instead of being checked, and only caught because the owner pushed). Every date, PR number, commit SHA, merge/deploy status, test count, and "LIVE"/"green"/"merged" word you write into a doc MUST be verified against the actual source **before** you write it — not from your own memory or your earlier chat messages. Run these and trust THEM: dates → `date +%Y-%m-%d` + `git --no-pager log --date=short --format='%h %ad %s' -8` (a session that spanned days must say so); PR/SHA/merge → `gh pr list --state all -L 5` + `git log --oneline`; CI/deploy/prod → `gh run view <id>` + `curl` the prod URL; a code claim → `grep`/read the file. This rule is the point of the whole ritual — a beautifully-formatted wrap-up full of remembered-but-wrong facts is worse than none.
...
3. **The FULL doc set → final MEASURED state** (not just the trackers — this is the step that was skipped and caused the rework). Update every one that applies:
   ...
   - [README.md](README.md) (and [web/README.md](web/README.md) if the web app changed) — if the one-page orientation now mis-states what the product does or how it's set up.
...
6. **Cleanup** — delete one-off scratch/verify scripts (incl. any `zzdel_*`/`__scratch*` review-agent test files that pollute the suite), keep the reusable ones committed; spin off out-of-scope hygiene as task chips.
7. **Next-session prompt** — hand a ready-to-paste kickoff (STATUS-first reading order, current state incl. the live/money-path caveat, the single next priority + its binding constraints/owner steers, model-tier/live-DDL/spend coordination notes, the #118 git-workflow reminder, the tracked-but-not-focus residuals) **AND save a durable copy to [docs/session-briefs/](docs/session-briefs/)`<date>-session-<n>-kickoff.md`** so the handoff survives a clean 0%-context restart (the chat may be gone; the repo is the source of truth).
8. **FINAL SELF-AUDIT** (do this LAST, before declaring wrapped) — re-read your own STATUS / status-archive / memory / lessons edits from this session and cross-check EVERY date, PR#, SHA, test count, and status word against the verified sources from the Golden Rule above. Fix any mismatch. Only after this passes may you declare the session wrapped — this step is the backstop that catches the one fact you got wrong from memory.
